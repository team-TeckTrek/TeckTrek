...
type Props = {
  onClose: () => void
  onConfirm?: (data: { name: string; iconIndex: number }) => void
}

type Step = 'form' | 'loading' | 'error'

export default function RandomCreateModal({ onClose, onConfirm }: Props) {
  ...
  const startMatching = () => {
    ...
    onConfirm?.({ name: trimmed, iconIndex: icon! })
  }
